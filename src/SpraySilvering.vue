<template>
  <div>
    ...
    <div class="card bg-base-200 p-3 mb-3">
      <h3 class="text-sm font-semibold mb-2">{{ $t('spraySilvering.bomTitle') }}</h3>
    ...
    <div class="card bg-base-200 p-3 mb-3">
      <div class="flex items-center gap-2 mb-2">
        <span class="badge badge-neutral badge-sm">{{ $t('spraySilvering.step0') }}</span>
        <h2 class="text-sm font-bold">{{ $t('spraySilvering.calibrateBottles') }}</h2>
      </div>
      <p class="text-xs">
        {{ $t('spraySilvering.calibrate') }}
        <span class="badge badge-neutral badge-xs">{{ $t('spraySilvering.bottle1') }}</span> and
        <span class="badge badge-error badge-xs">{{ $t('spraySilvering.bottle2') }}</span> {{ $t('spraySilvering.calibrateInstruction') }}
      </p>
    </div>
    <div class="card bg-base-200 p-3 mb-3">
      <div class="flex items-center gap-2 mb-2">
        <span class="badge badge-primary badge-sm">{{ $t('spraySilvering.step1') }}</span>
        <h2 class="text-sm font-bold">{{ $t('spraySilvering.prepareSolutions') }}</h2>
      </div>
      <div class="field-horizontal">
        <label class="label text-xs font-medium">{{ $t('spraySilvering.baseQuantity') }}</label>
        <input
          type="number"
    ...
    <div class="card bg-base-200 p-3 mb-3">
      <div class="space-y-2 text-xs">
        <p>
          <span class="badge badge-info badge-sm">{{ $t('spraySilvering.solution1') }}</span>:
          <strong>{{ silver }}g</strong> {{ $t('spraySilvering.silverNitrate') }} +
          <strong>{{ b }}ml</strong> {{ $t('spraySilvering.distilledWater') }}
        </p>
        <p>
          <span class="badge badge-success badge-sm">{{ $t('spraySilvering.solution2') }}</span>:
          <strong>{{ soda }}g</strong> {{ $t('spraySilvering.sodiumHydroxide') }} +
          <strong>{{ b }}ml</strong> {{ $t('spraySilvering.distilledWater') }}
        </p>
        <p>
          <span class="badge badge-warning badge-sm">{{ $t('spraySilvering.solution3') }}</span>:
          <strong>{{ sugar }}g</strong> {{ $t('spraySilvering.glucose') }} +
          <strong>{{ sugarwater }}ml</strong> {{ $t('spraySilvering.distilledWater') }}
        </p>
      </div>
    </div>
    ...
    <div class="alert alert-info mb-3 py-2">
      ...
      <div class="text-xs">
        <p>
          <span class="badge badge-neutral badge-xs">{{ $t('spraySilvering.bottle1') }}</span>
          {{ $t('spraySilvering.bottle1Info') }}
        </p>
        <p>
          <span class="badge badge-error badge-xs">{{ $t('spraySilvering.bottle2') }}</span>
          {{ $t('spraySilvering.bottle2Info') }}
          <span class="badge badge-warning badge-xs">{{ $t('spraySilvering.solution3') }}</span>
        </p>
      </div>
    </div>
    <div class="card bg-base-200 p-3 mb-3">
      <div class="flex items-center gap-2 mb-2">
        <span class="badge badge-secondary badge-sm">{{ $t('spraySilvering.step2') }}</span>
        <h2 class="text-sm font-bold">{{ $t('spraySilvering.mixingSilver') }}</h2>
      </div>
      <p>
        ({{ $t('spraySilvering.becomes') }} <span class="badge badge-neutral badge-xs">{{ $t('spraySilvering.bottle1') }}</span>)
      </p>
      <ul class="list-disc list-inside space-y-2 text-xs">
        <li>
          {{ $t('spraySilvering.step2_1') }} <strong>{{ firstQty }}ml</strong> {{ $t('spraySilvering.step2_1b') }}
          <span class="badge badge-info badge-xs">{{ $t('spraySilvering.solution1') }}</span>
        ...
        <li>
          ...
          <span class="badge badge-success badge-xs">{{ $t('spraySilvering.solution2') }}</span>
        </li>
        <li>
    ...
    <hr class="my-4" />
    <h2 class="font-bold text-xl mb-4">{{ $t('spraySilvering.cleaningTitle') }}</h2>
    ...
    <div class="rounded-md bg-gray-200 p-2">
      <label for="" class="label is-small"> {{ $t('spraySilvering.mirrorDiameter') }} </label>
      <input
  ...
  methods: {
    ...
    loadBomItems() {
      const defaultItems = [
        ...
        { name: "Anti-projection box", key: "bomAntiProjection", checked: false },
        ...
        { name: "A box in which to silver", key: "bomSilverBox", checked: false },
      ...
      if (savedItems) {
        ...
        defaultItems.forEach((item) => {
          if (!parsedItems.find((saved) => saved.name === item.name || saved.key === item.key)) {
